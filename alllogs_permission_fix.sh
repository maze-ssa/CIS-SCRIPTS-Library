...
{ 
    ...
    find /var/log -type f | while read -r fname; do 
        ...
        ! grep -Pq -- '^\h*(utmp|root)\h*$' <<< "$fugroup" && echo -e "- changing group on \"$fname\"" && chgrp root "$fname" ;; secure | auth.log | syslog | messages) 
        ...
        ! grep -Pq -- '^\h*(adm|root)\h*$' <<< "$fugroup" && echo -e "- changing group on \"$fname\"" && chgrp root "$fname" ;; SSSD | sssd) 
        ...
        ! grep -Piq -- '^\h*(SSSD|root)\h*$' <<< "$fugroup" && echo -e "- changing group on \"$fname\"" && chgrp root "$fname" ;; gdm | gdm3) 
        ! grep -Pq -- '^\h*[0,2,4,6][0,2,4,6]0\h*$' <<< "$fmode" && echo -e "- changing mode on \"$fname\"" && chmod ug-x,o-rwx 
        ! grep -Pq -- '^\h*root\h*$' <<< "$funame" && echo -e "- changing owner on \"$fname\"" && chown root "$fname" 
        ! grep -Pq -- '^\h*(gdm3?|root)\h*$' <<< "$fugroup" && echo -e "- changing group on \"$fname\"" && chgrp root "$fname" ;; *.journal | *.journal~) 
        ! grep -Pq -- '^\h*[0,2,4,6][0,4]0\h*$' <<< "$fmode" && echo -e "- changing mode on \"$fname\"" && chmod u-x,g-wx,o-rwx "$fname" 
        ! grep -Pq -- '^\h*root\h*$' <<< "$funame" && echo -e "- changing owner on \"$fname\"" && chown root "$fname" 
