...
{
	l_output="" l_output2=""
	l_kparameters="net.ipv4.ip_forward=0 net.ipv6.conf.all.forwarding=0" # Space
	separated list
	...
	for l_kpar in $l_kparameters; do
		...
		if grep -Pq '^\h*net\.ipv6\.' <<<"$l_kpname"; then
			if grep -Pqs '^\h*0\b' /sys/module/ipv6/parameters/disable; then
				...
				l_output="$l_output\n - IPv6 is not enabled, check for: \"$l_kpar\" is not
applicable"
			fi
		else
			kernel_par_chk
